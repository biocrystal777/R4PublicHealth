...
Die Dämpfungskonstante $d$ beträgt $d = \SI{0.25}{\cm}$ und die Schwingungsfrequenz $f = \SI{25}{\text{Hz}}$.
...
noch einen Abst von minimaler und maximaler Auslenkung $|\Delta I|(t)$ von mindestens \SI{6}{\min{cm}} aufweist?\\
Tipps: 
\begin{itemize}
...
\stepcounter{exnum}\paragraph{\arabic{exnum}}
Erstelle einen \texttt{Vector} $t$ und einen \texttt{Vector} $I$, der mit einer sample rate von \SI{1}{kHz} die 
Bewegung der Feder aus Aufgabe 10% \ref{subsec:DampedOsz}
beschreibt.
Erstelle einen weiteres \texttt{Vector}-Paar, der mit einer Abtastrate von $2f$ Minima und Maxima bestimmt.
In einem dritten \texttt{Vector}-Paar soll der erwarte Verlauf von $|\Delta I(t)|$ modelliert werden.
